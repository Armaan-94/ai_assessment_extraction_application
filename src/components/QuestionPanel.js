    ...
    <div className="w-full lg:w-[35%] h-1/2 lg:h-full shrink-0 bg-[#fafafa] border-b lg:border-b-0 lg:border-r border-gray-200 flex flex-col">
      <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-white shrink-0">
        ...
        <button
          ...
          className="text-xs font-medium px-3 py-1 bg-white border border-gray-200 rounded-full hover:bg-gray-50"
        >
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className={`text-sm text-gray-800 ${expandAll ? '' : 'line-clamp-3'}`}>{q.text}</p>

                  {(isSelected || expandAll) && grade && (
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  <svg className={`w-4 h-4 text-gray-400 transition-transform ${isSelected || expandAll ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                </div>
              </div>
            </div>
          );
